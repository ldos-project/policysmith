...
class WebCacheEvolve(EvolveInterface):
    ...
    def run_experiment(self):
        proc = subprocess.Popen(
            f"cd {self.build_dir} && ./run_algo.o {self.trace_path} {' '.join(map(str, self.task_args.cache_sizes))}",
            shell=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
        ...
        if success:
            ...
            relevant_result = list(filter(lambda x: x['percent'] == self.task_args.percent, results_list))
